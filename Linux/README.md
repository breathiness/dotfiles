...
- 闪电藤 - 基于 LocalSend 的聊天式本地传输工具

之前我都是使用 Feem 来进行本地文件传输。但是几点挺难受的地方。首先是所有端都有广告。然后Linux端在某次更新之后就打不开了。以及安卓端按退出键没办法退出。所以我一直都挺想拿个什么工具替换掉。其中 LocalSend 我之前有试用过。感觉操作逻辑有点怪怪的，还是不如聊天式的方式舒服。但在最近发现了这个基于LocalSend二次开发的闪电藤。继承了多平台的特性，以及相关的各种设置。但是传输方式改成了类似 Feem 的聊天式传输。
目前使用下来是没什么可吐槽的地方。就是账号功能没什么用，还有个私服管理的功能在开发中，不知道有什么用。还是挺期待的。

项目链接：<https://lightningvine.zishu.life>
