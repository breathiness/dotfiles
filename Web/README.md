...
项目链接：<https://greasyfork.org/zh-CN/scripts/33268-katakana-terminator>
    ...
    - ​Novel-Downloader 一个可扩展的通用型小说下载器

我前段时间挺常在某个网站看小说，但是上面的投稿经常会因为各种奇怪的原因消失。于是我需要找到一个可以将网页小说下载到本地的方式。
早期的时候我是使用手机软件的下载功能。虽然是可以下载文字部分，但是下载的结果缺少了章节分割，以及文章的配图。虽然不太影响阅读。但是总还是有点不爽。
然后某天在 GitHub 闲逛时，突然看到了这个油猴脚本。支持站点非常多；可以下载多种格式（我推荐EPUB）；并且可以下载封面、原始网址、文章内部图片、章节划分、简介、标签；并且这些简介标签之类的可以直接被我在用的书籍管理软件Caliber读取。节省了大量的后期处理的时间。
目前的缺点是某些站点一次性下载太多的话会被反爬检测到。要注意使用频率别太快。

项目链接：<https://github.com/404-novel-project/novel-downloader>
